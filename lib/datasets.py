...
class ATPBind(data.ProteinDataset):
    ...
    def __init__(self, path, verbose=1, valid_ratio=0.1, **kwargs):
        self.num_samples = []
    ...
    def get_seq_target(self, path):
        ...
        self.num_samples=[self.num_samples[0]-val_num,val_num,self.num_samples[1]]
    ...
    def get_item(self, index):
        ...
        with graph.residue():
            target = torch.as_tensor(self.targets["binding"][index], dtype=torch.long).view(-1, 1)
            graph.target = target
